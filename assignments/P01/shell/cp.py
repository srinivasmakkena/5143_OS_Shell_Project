"""
  ...
  | |   | |_) | | |  | | | | |\/| | |\/| | / _ \ |  \| | | | |
  | |___|  __/  | |__| |_| | |  | | |  | |/ ___ \| |\  | |_| |
   \____|_|      \____\___/|_|  |_|_|  |_/_/   \_\_| \_|____/ 
...
import datetime
def cp(command, shell_obj,output_dict):
    ...
    if source_path and dest_path :
        ...
        if len(shell_obj.file_model.objects(parent_id = temp_dir.id, filename = file_or_dir).all()):
            source_obj = shell_obj.file_model.objects(parent_id = temp_dir.id, filename = file_or_dir).all()[0]
        elif file_or_dir == ".":
            source_obj = temp_dir
        ...
        if source_obj.metadata["File Type"]!= "Directory":
            ...
            if obj:
                # obj = shell_obj.file_model.objects(filename=file_or_dir, parent_id = temp_dir.id).all()[0]
            ...
            output_dict["error"] = "Copying folders in recursive way is not possible: Try copying files to folders or files."
            return output_dict      
